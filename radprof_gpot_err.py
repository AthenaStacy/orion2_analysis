...
for i in range(narr):	
        if i < 1:
                gaccxL[i] = (np.abs(gpotxR[1] - gpotxL[0]) / DeltaX)
                gaccxR[i] = (np.abs(gpotxL[1] - gpotxR[0]) / DeltaX)
                gaccyL[i] = (np.abs(gpotyR[1] - gpotyL[0]) / DeltaX)
                gaccyR[i] = (np.abs(gpotyL[1] - gpotyR[0]) / DeltaX)
                gacczL[i] = (np.abs(gpotzR[1] - gpotzL[0]) / DeltaX)
                gacczR[i] = (np.abs(gpotzL[1] - gpotzR[0]) / DeltaX)
        if i >= 1:
                gaccxL[i] = (  np.abs(gpotxL[i] - gpotxL[i-1]) / DeltaX )
                gaccxR[i] = (  np.abs(gpotxR[i] - gpotxR[i-1]) / DeltaX )
                gaccyL[i] = (  np.abs(gpotyL[i] - gpotyL[i-1]) / DeltaX )
                gaccyR[i] = (  np.abs(gpotyR[i] - gpotyR[i-1]) / DeltaX )
                gacczL[i] = (  np.abs(gpotzL[i] - gpotzL[i-1]) / DeltaX )
                gacczR[i] = (  np.abs(gpotzR[i] - gpotzR[i-1]) / DeltaX )

for i in range(narr):
        gaccxLF.append( np.abs(gpotxL[narr-1] - gpotxR[narr-1]) / radpot[i] )
        gaccxRF.append( np.abs(gpotxR[narr-1] - gpotxL[narr-1]) / radpot[i] )
        gaccyLF.append( np.abs(gpotyL[narr-1] - gpotyR[narr-1]) / radpot[i] )
        gaccyRF.append( np.abs(gpotyR[narr-1] - gpotyL[narr-1]) / radpot[i] )
        gacczLF.append( np.abs(gpotzL[narr-1] - gpotzR[narr-1]) / radpot[i] )
        gacczRF.append( np.abs(gpotzR[narr-1] - gpotzL[narr-1]) / radpot[i] )
##############################################################################
...
for i in range(bin_num_square):
        if i == 0:
                ...
                gaccxL_arr.append(np.abs(gpotxL_arr[1] - gpotxL_arr[0]) / DeltaX )
                gaccxR_arr.append(np.abs(gpotxR_arr[1] - gpotxR_arr[0]) / DeltaX)
                gaccyL_arr.append(np.abs(gpotyL_arr[1] - gpotyL_arr[0]) / DeltaX)
                gaccyR_arr.append(np.abs(gpotyR_arr[1] - gpotyR_arr[0]) / DeltaX)
                gacczL_arr.append(np.abs(gpotzL_arr[1] - gpotzL_arr[0]) / DeltaX)
                gacczR_arr.append(np.abs(gpotzR_arr[1] - gpotzR_arr[0]) / DeltaX)
        if i > 0:
...
for i in range(1,bin_num_square):
        if gpot_arr[i] < gpot_arr_min:
		gpot_arr_min = gpot_arr[i]
        if gpotB[i] < gpotB_min:
		gpotB_min = gpotB[i]
        if gpotxL_arr[i] < gpotxL_arr_min:
                gpotxL_arr_min = gpotxL_arr[i]
        if gpotxR_arr[i] < gpotxR_arr_min:
                gpotxR_arr_min = gpotxR_arr[i]
        if gpotyL_arr[i] < gpotyL_arr_min:
                gpotyL_arr_min = gpotyL_arr[i]
        if gpotyR_arr[i] < gpotyR_arr_min:
                gpotyR_arr_min = gpotyR_arr[i]
        if gpotzL_arr[i] < gpotzL_arr_min:
                gpotzL_arr_min = gpotzL_arr[i]
        if gpotzR_arr[i] < gpotzR_arr_min:
...
for i in range(bin_num_square):
	...
	gaccxL_err.append(np.abs(gaccxL_arr[i] - gaccxL[i])/gaccxL[i])
        gaccxR_err.append(np.abs(gaccxR_arr[i] - gaccxR[i])/gaccxR[i])
        gaccyL_err.append(np.abs(gaccyL_arr[i] - gaccyL[i])/gaccyL[i])
	gaccyR_err.append(np.abs(gaccyR_arr[i] - gaccyR[i])/gaccyR[i])
        gacczL_err.append(np.abs(gacczL_arr[i] - gacczL[i])/gacczL[i])
        gacczR_err.append(np.abs(gacczR_arr[i] - gacczR[i])/gacczR[i])
        gaccxL_err_pn.append((gaccxL_arr[i] - gaccxL[i])/gaccxL[i])
        gaccxR_err_pn.append((gaccxR_arr[i] - gaccxR[i])/gaccxR[i])
        gaccyL_err_pn.append((gaccyL_arr[i] - gaccyL[i])/gaccyL[i])
        gaccyR_err_pn.append((gaccyR_arr[i] - gaccyR[i])/gaccyR[i])
        gacczL_err_pn.append((gacczL_arr[i] - gacczL[i])/gacczL[i])
        gacczR_err_pn.append((gacczR_arr[i] - gacczR[i])/gacczR[i])
...
pl.plot(rad_neg, gaccxL_err, 'r--')
pl.plot(rad_arr, gaccxR_err, 'r--')
pl.plot(rad_neg, gaccyL_err, 'g--')
pl.plot(rad_arr, gaccyR_err, 'g--')
pl.plot(rad_neg, gacczL_err, 'b--')
pl.plot(rad_arr, gacczR_err, 'b--')
ax = pl.gca()
ax.set_yscale('log')
ax.set_xlabel('Radius [AU]', fontsize=9)
ax.set_ylabel('acceleration error pot.', fontsize=9)
pl.xticks(fontsize=10)
pl.yticks(fontsize=10)
pl.axis((rmin, rmax, 1.e-3, 1.e1))

pl.savefig('plot_err.ps')
