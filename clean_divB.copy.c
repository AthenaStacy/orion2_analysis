...
int main(int argc, char **argv)
{
  ...
  sprintf(output_fname, "%s/gadget2orion_bfield", path_out);
...
for(igrid=0; igrid<gnum; igrid++)
 for(jgrid=0; jgrid<gnum; jgrid++)
   for(kgrid=0; kgrid<gnum; kgrid++)
     ...
     divB_avg = divB_avg + grid[n].DivB[igrid][jgrid][kgrid];
...
for(igrid=0; igrid<gnum; igrid++)
 //for(jgrid=0; jgrid<gnum; jgrid++)
   //for(kgrid=0; kgrid<gnum; kgrid++)
     ...
     gphi_xplus = 2.*grid[n].DivB[igrid][jgrid][kgrid] * DeltaX - (gphi_zplus + gphi_zmin)  - (gphi_yplus + gphi_ymin) - gphi_xmin + 6.*grid[n].grad_phi[igrid][jgrid][kgrid];
     //grid[n].grad_phi[i_hi][jgrid][kgrid] = 2. * gphi_xplus - grid[n].grad_phi[igrid][jgrid][kgrid];
     grid[n].grad_phi[i_hi][jgrid][kgrid] = gphi_xplus; 

/*
     gphi_yplus = 2.*grid[n].DivB[igrid][jgrid][kgrid] * DeltaX - (gphi_xplus + gphi_xmin)  - (gphi_zplus + gphi_zmin) - gphi_ymin +  6.*grid[n].grad_phi[igrid][jgrid][kgrid];
     grid[n].grad_phi[igrid][j_hi][kgrid] = gphi_yplus;

     gphi_zplus = 2.*grid[n].DivB[igrid][jgrid][kgrid] * DeltaX - (gphi_xplus + gphi_xmin)  - (gphi_yplus + gphi_ymin) - gphi_zmin +  6.*grid[n].grad_phi[igrid][jgrid][kgrid];
     grid[n].grad_phi[igrid][jgrid][k_hi] =  gphi_zplus;
*/
     if(myrank == 0) printf("grid[n].grad_phi[%d][%d][%d] = %lg, gphi_xplus = %lg, gphi_xmin = %lg\n", igrid, jgrid, kgrid, grid[n].grad_phi[igrid][jgrid][kgrid], gphi_xplus, gphi_xmin);
     }  
...
//for(igrid=0; igrid<gnum; igrid++)
 for(jgrid=0; jgrid<gnum; jgrid++)
   //for(kgrid=0; kgrid<gnum; kgrid++)
     ...
     gphi_yplus = 2.*grid[n].DivB[igrid][jgrid][kgrid] * DeltaX - (gphi_xplus + gphi_xmin)  - (gphi_zplus + gphi_zmin) - gphi_ymin +  6.*grid[n].grad_phi[igrid][jgrid][kgrid];
     //grid[n].grad_phi[igrid][j_hi][kgrid] = 2. * gphi_yplus - grid[n].grad_phi[igrid][jgrid][kgrid];
     grid[n].grad_phi[igrid][j_hi][kgrid] = gphi_yplus;

    if(myrank == 0) printf("grid[n].grad_phi[%d][%d][%d] = %lg, gphi_yplus = %lg, gphi_ymin = %lg\n", igrid, jgrid, kgrid, grid[n].grad_phi[igrid][jgrid][kgrid], gphi_yplus, gphi_ymin);
     }
...
// for(jgrid=0; jgrid<gnum; jgrid++)
   for(kgrid=0; kgrid<gnum; kgrid++)
     ...
     gphi_zplus = 2.*grid[n].DivB[igrid][jgrid][kgrid] * DeltaX - (gphi_xplus + gphi_xmin)  - (gphi_yplus + gphi_ymin) - gphi_zmin +  6.*grid[n].grad_phi[igrid][jgrid][kgrid];
     //grid[n].grad_phi[igrid][jgrid][k_hi] = 2. * gphi_zplus - grid[n].grad_phi[igrid][jgrid][kgrid];
     grid[n].grad_phi[igrid][jgrid][k_hi] =  gphi_zplus;

    if(myrank == 0) printf("grid[n].grad_phi[%d][%d][%d] = %lg, gphi_zplus = %lg, gphi_zmin = %lg\n", igrid, jgrid, kgrid, grid[n].grad_phi[igrid][jgrid][kgrid], gphi_zplus, gphi_zmin);
     }
...
if(myrank == 0) printf("B Bx_new = %lg, By_new = %lg, Bz_new = %lg\n", grid[n].Bx_new[10][10][10], grid[n].By_new[10][10][10], grid[n].Bz_new[10][10][10]);
if(myrank == 0) printf("Bx = %lg, By = %lg, Bz = %lg\n", grid[n].Bfieldx[10][10][10], grid[n].Bfieldy[10][10][10], grid[n].Bfieldz[10][10][10]);
...
if(myrank == 0) printf("C Bx_new = %lg, By_new = %lg, Bz_new = %lg\n", grid[n].Bx_new[10][10][10], grid[n].By_new[10][10][10], grid[n].Bz_new[10][10][10]);
if(myrank == 0) printf("Bx = %lg, By = %lg, Bz = %lg\n", grid[n].Bfieldx[10][10][10], grid[n].Bfieldy[10][10][10], grid[n].Bfieldz[10][10][10]);
...
for(igrid=0; igrid<gnum; igrid++)
 //for(jgrid=0; jgrid<gnum; jgrid++)
   //for(kgrid=0; kgrid<gnum; kgrid++)
     ...
     gphi_xplus = 1.*(grid[n].Bfieldx[igrid][jgrid][kgrid] -  grid[n].Bx_new[igrid][jgrid][kgrid]) + gphi_xmin; 
     grid[n].grad_phi[i_hi][jgrid][kgrid] = 2. * gphi_xplus - grid[n].grad_phi[igrid][jgrid][kgrid];
     }
...
//for(igrid=0; igrid<gnum; igrid++)
 for(jgrid=0; jgrid<gnum; jgrid++)
   //for(kgrid=0; kgrid<gnum; kgrid++)
     ...
     gphi_yplus = 1.*(grid[n].Bfieldy[igrid][jgrid][kgrid] -  grid[n].By_new[igrid][jgrid][kgrid]) + gphi_ymin;
     grid[n].grad_phi[igrid][j_hi][kgrid] = 2. * gphi_yplus - grid[n].grad_phi[igrid][jgrid][kgrid];
     }
...
//for(igrid=0; igrid<gnum; igrid++)
 //for(jgrid=0; jgrid<gnum; jgrid++)
   for(kgrid=0; kgrid<gnum; kgrid++)
     ...
     gphi_zplus = 1.*(grid[n].Bfieldz[igrid][jgrid][kgrid] -  grid[n].Bz_new[igrid][jgrid][kgrid]) + gphi_zmin;
     grid[n].grad_phi[igrid][jgrid][k_hi] = 2. * gphi_zplus - grid[n].grad_phi[igrid][jgrid][kgrid];
     }
...
for(igrid=0; igrid<gnum; igrid++)
 for(jgrid=0; jgrid<gnum; jgrid++)
   for(kgrid=0; kgrid<gnum; kgrid++)
     ...
     divB_avg = divB_avg + divB_cur;
     B_new_avg = B_avg + pow(Bx_new*Bx_new + By_new*By_new + Bz_new*Bz_new, 0.5);
     B2_avg = B2_avg + pow(Bx*Bx + By*By + Bz*Bz, 0.5);
     }

if(myrank == 0) printf("New DivB! divB_avg = %lg, B2_avg = %lg, B_new_avg = %lg\n", divB_avg/pow(gnum,3), B2_avg/pow(gnum,3), B_new_avg/pow(gnum,3));
