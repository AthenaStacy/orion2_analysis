...
for(igrid=0; igrid<gnum; igrid++)
 for(jgrid=0; jgrid<gnum; jgrid++)
   for(kgrid=0; kgrid<gnum; kgrid++)
     ...
     get_gphi_alt(n);
     gphi_xplus = 2.*(grid[n].Bfieldx[igrid][jgrid][kgrid] -  grid[n].Bx_new[igrid][jgrid][kgrid]) + gphi_xmin;
...
for(igrid=0; igrid<gnum; igrid++)
 for(jgrid=0; jgrid<gnum; jgrid++)
   for(kgrid=0; kgrid<gnum; kgrid++)
     ...
     get_gphi_alt(n);
     gphi_yplus = 2.*(grid[n].Bfieldy[igrid][jgrid][kgrid] -  grid[n].By_new[igrid][jgrid][kgrid]) + gphi_ymin;
...
for(igrid=0; igrid<gnum; igrid++)
 for(jgrid=0; jgrid<gnum; jgrid++)
   for(kgrid=0; kgrid<gnum; kgrid++)
     ...
     get_gphi_alt(n);
     gphi_zplus = 2.*(grid[n].Bfieldz[igrid][jgrid][kgrid] -  grid[n].Bz_new[igrid][jgrid][kgrid]) + gphi_zmin;
