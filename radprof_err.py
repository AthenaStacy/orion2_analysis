...
with open(rdir + file_prefix + '_gas_' + snapnum +'.dat.lin', "r") as f:
...
p1b = pc.add_profile_sphere(1., "pc", ['Radius','radial-velocity','rot-velocity'], weight='CellMassMsun', x_bins=bin_num2, x_log=True, x_bounds = [rmin,rmax], center = location)

p1c = pc.add_profile_sphere(1., "pc", ['Radius', 'Pressure'], weight='CellMassMsun', x_bins=bin_num1, x_log=False, x_bounds = [rmin,rmax], center = location)
...
for i in range(narr-1):
        print rad[i], radB[i], nh[i],nhB[i],  massenc_arr[i], mencB[i]
	if nh[i] > 0:
		rad_err.append(rad[i])
		nh_err.append((nh[i] - nhB[i])/nhB[i])
        	dens_err.append((dens[i] - densB[i])/densB[i])
		nh2_err.append((nh2[i] - nhB[i])/nhB[i])
		dens2_err.append((dens2[i] - densB[i])/densB[i])
		mass_err.append((massenc_arr[i] - mencB[i])/mencB[i])
		temp_err.append((temp[i] - tempB[i])/tempB[i])
		vrad_err.append((vrad[i] - vradB[i])/vradB[i])

#for i in range(narr-100):
#        print rad_err[i], nh2[i], nhB[i],  massenc_arr[i], mencB[i]
...
pl.subplot(221)
#pl.plot(rad_err, nh_err)
pl.plot(rad_err, dens2_err)
ax = pl.gca()
ax.set_xscale('log')
#ax.set_yscale('log')
ax.set_xlabel('Radius [AU]', fontsize=9)
ax.set_ylabel('number density error', fontsize=9)
pl.xticks(fontsize=10)
pl.yticks(fontsize=10)
pl.axis((rmin, rmax, errmin_dens, errmax_dens))

if setup== 3:
        pl.subplot(222)
        pl.plot(rad_err, temp_err)
        ax = pl.gca()
        ax.set_xscale('log')
        #ax.set_yscale('log')
        ax.set_xlabel('Radius [AU]', fontsize=9)
...
if setup== 3:
        pl.subplot(223)
        pl.plot(rad_err, vrad_err)
        ax = pl.gca()
        ax.set_xscale('log')
        #ax.set_yscale('log')
        ax.set_xlabel('Radius [AU]', fontsize=9)
        ax.set_ylabel('radial-velocity error', fontsize=9)
        pl.xticks(fontsize=10)
        pl.yticks(fontsize=10)
        pl.axis((rmin, rmax, errmin, errmax))
...
if setup == 2 or setup == 3:
	...
	pl.axis((rmin, rmax, errmin_mass, errmax_mass))
