...
//#define which_snap 200
//#define which_snap 827     //bin_HR10_ideal
...
int main(int argc, char **argv)
{
  ...
  if(which_sim == 3)
   ...
   if(which_sim == 4)
    ...
    sprintf(path_in, "/work/00863/minerva/");
    sprintf(path_out, "/work/00863/minerva/orion");
    if(convert_vpot == 1) sprintf(path_out, "/work/00863/minerva/orion/bfield_comp_vpot");
...
*/
  ...
  if(which_sim == 3)
    sprintf(output_fname, "%s/gadget2orion_bfield_ref3_%05d", path_out, snapshot_number);
...
int write_snapshot(char *fname, int files, char *outname, char *outname0, char *outname1, char *outname2, char *outname3, char *outname4, char *outname8, char *outname9, char *outname10, double delx, double dely, double delz, double delvx, double delvy, double delvz, int NumGas, int myrank)
{
  ...
  double hsm_conv, vel_conv, dens_conv, mass_conv, kernel_conv, kernel_phys, mass, rho, hsm; 
  ...
  double *Gdum, *Gdumx, *Gdumy, *Gdumz, *Grho, *Grho_tot, *Gvelx, *Gvely, *Gvelz, *Gpres, *GH2I, *GHDI, *GHII; 
          ...
          for(j=jmin;j<=jmax;j++)
            for(k=kmin;k<=kmax;k++)
               ...
               if(vartype == 0)
                 ...
                 //fac = pow(grid_size*3.0857e18, -3) / kernel_phys;
                 ...
                 if(kernel_phys > 0 && n%100000000 == 0)
      ...
      for(i=0;i<ref_lev;i++)
        for(j=0;j<ref_lev;j++)
          for(k=0;k<ref_lev;k++)
             ...
             if(vartype > 0)  Gtot[l] = (Gtot[l]/Grho_tot[l]);  //write out stuff that is NOT density!
