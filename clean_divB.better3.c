...
for(igrid=0; igrid<gnum; igrid++)
 for(jgrid=0; jgrid<gnum; jgrid++)
   for(kgrid=0; kgrid<gnum; kgrid++)
     ...
     if(igrid == gnum-1 && i_hi == gnum-1) continue;
     if(jgrid == gnum-1 && j_hi == gnum-1) continue;
     if(kgrid == gnum-1 && k_hi == gnum-1) continue;
     if(igrid == 0 && i_lo == 0) continue;
     if(jgrid == 0 && j_lo == 0) continue;
     if(kgrid == 0 && k_lo == 0) continue;
...
for(igrid=0; igrid<gnum; igrid++)
 for(jgrid=0; jgrid<gnum; jgrid++)
   for(kgrid=0; kgrid<gnum; kgrid++)
     ...
     grid[n].grad_phi[igrid][0][kgrid] = 0;
     grid[n].grad_phi[igrid][gnum-1][kgrid] = 0;

     grid[n].grad_phi[igrid][jgrid][0] = 0;
     grid[n].grad_phi[igrid][jgrid][gnum-1] = 0;
...
for(igrid=0; igrid<gnum; igrid++)
 for(jgrid=0; jgrid<gnum; jgrid++)
   for(kgrid=0; kgrid<gnum; kgrid++)
     ...
     Bx_left = Bx_right + (By_right - By_left) - (Bz_right - Bz_left);
     if(interp_cell == 1)
       grid[n].Bx_new[i_lo][jgrid][kgrid] = 2. * Bx_left - grid[n].Bx_new[igrid][jgrid][kgrid];
     else 
       grid[n].Bx_new[i_lo][jgrid][kgrid] = Bx_left;

     get_gphi_alt(n);
     gphi_xmin = gphi_xplus - 2.*(grid[n].Bfieldx[igrid][jgrid][kgrid] -  grid[n].Bx_new[igrid][jgrid][kgrid]);
     grid[n].grad_phi[i_lo][jgrid][kgrid] =  gphi_xmin;

     if(interp_cell == 1) get_Bnew(n);  else get_Bnew_alt(n);

     By_left = By_right + (Bx_right - Bx_left) - (Bz_right - Bz_left);
     if(interp_cell == 1)
       grid[n].By_new[igrid][j_lo][kgrid] = 2. * By_left - grid[n].By_new[igrid][jgrid][kgrid];
     else
       grid[n].By_new[igrid][j_lo][kgrid] = By_left;

     get_gphi(n);
     //get_gphi_alt(n);
     gphi_ymin = gphi_yplus - 2.*(grid[n].Bfieldy[igrid][jgrid][kgrid] -  grid[n].By_new[igrid][jgrid][kgrid]);
     grid[n].grad_phi[igrid][j_lo][kgrid] = gphi_ymin;

     if(interp_cell == 1) get_Bnew(n);  else get_Bnew_alt(n);

     Bz_left = Bz_right + (Bx_right - Bx_left) - (By_right - By_left);
     if(interp_cell == 1)
       grid[n].Bz_new[igrid][jgrid][k_lo] = 2. * Bz_left - grid[n].Bz_new[igrid][jgrid][kgrid];
     else 
       grid[n].Bz_new[igrid][jgrid][k_lo] = Bz_left;

     get_gphi(n);
     //get_gphi_alt(n);
     gphi_zmin = gphi_zplus - 2.*(grid[n].Bfieldz[igrid][jgrid][kgrid] -  grid[n].Bz_new[igrid][jgrid][kgrid]);
     grid[n].grad_phi[igrid][jgrid][k_lo] = gphi_zmin;
     }
...
*/
     ...
     if(interp_cell == 1)
       grid[n].Bz_new[igrid][jgrid][k_hi] = 2. * Bz_right - grid[n].Bz_new[igrid][jgrid][kgrid];
     else 
...
*/
     ...
     if(interp_cell == 1)
       grid[n].By_new[igrid][j_hi][kgrid] = 2. * By_right - grid[n].By_new[igrid][jgrid][kgrid];
     else
       grid[n].By_new[igrid][j_hi][kgrid] = By_right;
     }


//Make divergence exactly zero
for(igrid=0; igrid<gnum; igrid++)
 for(jgrid=0; jgrid<gnum; jgrid++)
   for(kgrid=0; kgrid<gnum; kgrid++)
     {
...
*/
     ...
     if(interp_cell == 1)
       grid[n].Bx_new[i_hi][jgrid][kgrid] = 2. * Bx_right - grid[n].Bx_new[igrid][jgrid][kgrid];
     else if(interp_cell == 2) 
       grid[n].Bx_new[igrid][jgrid][kgrid] = Bx_right;
     else 
       grid[n].Bx_new[i_hi][jgrid][kgrid] = Bx_right;
     }

for(igrid=0; igrid<1; igrid++)
 for(jgrid=0; jgrid<gnum; jgrid++)
   for(kgrid=0; kgrid<gnum; kgrid++)
     {
     get_hilo();
     ...
     if(interp_cell == 1) get_Bnew(n);  else get_Bnew_alt(n);
     grid[n].Bx_new[igrid][jgrid][kgrid]  =  2. * Bx_right - grid[n].Bx_new[i_hi][jgrid][kgrid]; 
     }
...
*/
     ...
     grid[n].Bx_new_face[igrid][jgrid][kgrid] = Bx_left;
     grid[n].By_new_face[igrid][jgrid][kgrid] = By_left;
     grid[n].Bz_new_face[igrid][jgrid][kgrid] = Bz_left;
...
}while(fabs(B_avg_prev - B_new_avg) / B_new_avg > 1.e-6);
//while(fabs(B_avg - B_new_avg) / B_avg > 0.1);
//while(divB_avg/pow(gnum,3) > 1.e-50);
//while(B_new_avg < 1.e20);
...
fclose(gadget_out);
     ...
     for(i2=30;i2<gnum;i2++)
       for(j2=30;j2<gnum;j2++)
         for(k2=30;k2<gnum;k2++)
           {
           printf("face value: i2 = %d, j2 = %d, k2 = %d, Bfieldx = %lg\n", i2, j2, k2, grid[n].Bfieldx[i2][j2][k2] );
           printf("face value: i2 = %d, j2 = %d, k2 = %d, Bfieldy = %lg\n", i2, j2, k2, grid[n].Bfieldy[i2][j2][k2] );
           printf("face value: i2 = %d, j2 = %d, k2 = %d, Bfieldz = %lg\n", i2, j2, k2, grid[n].Bfieldz[i2][j2][k2] );
           printf("face value: i2 = %d, j2 = %d, k2 = %d, Bfieldx_face = %lg\n", i2, j2, k2, grid[n].Bx_new_face[i2][j2][k2] );
           printf("face value: i2 = %d, j2 = %d, k2 = %d, Bfieldy_face = %lg\n", i2, j2, k2, grid[n].By_new_face[i2][j2][k2] );
           printf("face value: i2 = %d, j2 = %d, k2 = %d, Bfieldz_face = %lg\n", i2, j2, k2, grid[n].Bz_new_face[i2][j2][k2] );
           }
