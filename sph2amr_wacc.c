...
#define width  0.5 //size of box in pc to be "cut out" of Gadget2 simulation box and mapped onto the Orion2 grid 
...
int write_snapshot(char *fname, int files, char *outname, char *outname2, char *outname3, char*outname4, double delx, double dely, double delz, double delvx, double delvy, double delvz, int NumGas, int proc);
int reordering(void);
...
{
  char path_in[200], path_out[200], input_fname[200], input_fname2[200], output_fname[200], output_fname2[200],  output_fname3[200],  output_fname4[200], basename[200], basenameout[200];
  ...
  FILE *outfile, *outfile2, *outfile3, *outfile4, *infile;
  ...
  sprintf(output_fname, "%s/gadget2orion_wacc", path_out);
  sprintf(output_fname2, "%s/gadget2accX", path_out);
  sprintf(output_fname3, "%s/gadget2accY", path_out);
  sprintf(output_fname4, "%s/gadget2accZ", path_out);
  ...
  if(myrank == 0)
    ...
    fclose(outfile2);
    outfile3=fopen(output_fname3,"w");
    fclose(outfile3);
    outfile4=fopen(output_fname4,"w");
    fclose(outfile4);
    }


  Ngas2 = write_snapshot(input_fname, files, output_fname, output_fname2, output_fname3, output_fname4, delx, dely, delz, delvx, delvy, delvz, Ngas, myrank);
...
int unit_conversion(void)
{
  ...
  acc_fac = (UnitLength_in_cm / header1.HubbleParam) / pow(UnitTime_in_s / header1.HubbleParam,2);
...
int write_snapshot(char *fname, int files, char *outname, char *outname2, char *outname3, char *outname4, double delx, double dely, double delz, double delvx, double delvy, double delvz, int NumGas, int myrank)
{
  ...
  double masstot_alt=0, xmomtot_alt=0, ymomtot_alt=0, zmomtot_alt=0, etot_alt=0, hfac = 1.0;
            ...
            for(k=kmin;k<=kmax;k++)
               ...
               if(vartype > 0 && Grho[l] > 0)
                 ...
                 fac = mass / P[n].mass_mapped;
                  ...
                  if(vartype == 9 && rho > 0)
                    Gdum[l] = Gdum[l] + P[n].Acc[0]*rho*fac;
                  if(vartype == 10 && rho > 0)
                    Gdum[l] = Gdum[l] + P[n].Acc[1]*rho*fac;
                  if(vartype == 11 && rho > 0)
                    Gdum[l] = Gdum[l] + P[n].Acc[2]*rho*fac;
                  }
               }
      ...
      for(i=0;i<ref_lev;i++)
        for(j=0;j<ref_lev;j++)
          for(k=0;k<ref_lev;k++)
             ...
             if(vartype == 9)  fwrite(&Gtot[l], sizeof(double), 1, outfile2);
             if(vartype == 10)  fwrite(&Gtot[l], sizeof(double), 1, outfile3);
             if(vartype == 11)  fwrite(&Gtot[l], sizeof(double), 1, outfile4); 
