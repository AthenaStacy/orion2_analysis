...
pf2 = load(rdir  + 'data.' + datanum2 + '.3d.hdf5')
...
c = 1.0*pf.domain_right_edge[slicedirection]
sl = pf.h.slice(slicedirection,c) ##Get the Slice
frac = 0.1
w = [frac*pf.domain_left_edge[xdir],frac*pf.domain_right_edge[xdir],frac*pf.domain_left_edge[ydir],pf.domain_right_edge[ydir]] #Choose the width, here I choose the entire domain
frb1 = FixedResolutionBuffer(sl,w,(res,res))  #Create FixedResolution Buffer

##Do the same (THIS CAN BE A DIFFERENT DATASET)
c = 1.0*pf2.domain_right_edge[slicedirection]
...
median1 = np.median(frb1[plotme])
print 'median val1 =', median1
print 'length val1 =', len(val1)
median2 = np.median(frb2[plotme])
print 'median val2 =', median2
print 'length val2 =', len(val2)
