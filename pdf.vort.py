...
datanum1  = '0016'  #--corresponding times and data steps for orion256
datanum1a = '0016'
...
for i in range(ngrid_xA):
        for j in range(ngrid_yA):
                for k in range(ngrid_zA):
                        for l in range(bin_num-1):
                                if array1A[i,j] > array_bin[l] and array1A[i,j] < array_bin[l+1]:
                                        numA[l] = numA[l] + 1
                                if i%10 == 0 and j%10 == 0 and k == 0:
                                        print 'i =', i, 'j =', j, 'k =', k
