...
for i in range (file_start, file_end, 20):
	datanum = '0' + str(i)
	...
	imax = np.argmax(mrat_max)
	mrat_max_max = mrat_max[imax]
	rho_max_max = rho_max[imax]
	temp_max_max = temp_max[imax]
	mcrit_max_max = mcrit_max[imax]

	imax = np.argmax(mrat_max0)
	mrat_max_max0 = mrat_max0[imax]
	rho_max_max0 = rho_max0[imax]
	temp_max_max0 = temp_max0[imax]
	mcrit_max_max0 = mcrit_max0[imax]

	f = open("mrat_l8.txt", "a")
	line = str(i) + ' ' + str(time.value) + ' ' + str(mrat_max_max) + ' ' + str(mcrit_max_max) + ' ' + str(rho_max_max) + ' ' + str(temp_max_max)
	f.write(line)
	f.write('\n')
	f.close()

	f = open("mrat0_l8.txt", "a")
	line = str(i) + ' ' + str(time0.value) + ' ' + str(mrat_max_max0) + ' ' + str(mcrit_max_max0) + ' ' + str(rho_max_max0) + ' ' + str(temp_max_max0)
	f.write(line)
	f.write('\n')
	f.close()
