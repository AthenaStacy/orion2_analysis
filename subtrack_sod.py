...
#Give me a file name
pf  = load("/work/00863/minerva/orion/sod_maptest1/" + 'data.' + datanum + '.3d.hdf5')
pf2 = load("/work/00863/minerva/orion/sod_maptest2/" + 'data.' + datanum + '.3d.hdf5')
...
for i in range(len(val1)):
    for j in range(len(val1)):
        ...
        error = (val1[i,j] - val2[i,j])/((val1[i,j]+val2[i,j])/2)
        if error > 1.e-3 and val1[i,j] < median:
