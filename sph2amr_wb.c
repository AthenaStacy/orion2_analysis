...
int main(int argc, char **argv)
{
  ...
  if(readB == 1)
    {
    sprintf(input_fname2, "%s/%s_bfield.dat", path_out, basename2);
    ...
    for(n=0;n<Ngas;n++)
        fread(&P[n].Bfieldx, sizeof(double), 3, infile);
   for(n=0;n<Ngas;n++)
        fread(&P[n].Bfieldy, sizeof(double), 3, infile);
   for(n=0;n<Ngas;n++)
        fread(&P[n].Bfieldz, sizeof(double), 3, infile);
    for(n=0;n<Ngas;n++)
...
int write_snapshot(char *fname, int files, char *outname, double delx, double dely, double delz, double delvx, double delvy, double delvz, int NumGas, int myrank)
{
  ...
  int NumGas_per_proc, vartype, varnum=12;
